...
# Enable key-based authentication & password authentication
echo "PasswordAuthentication yes" >> %{buildroot}%{_sysconfdir}/ssh/sshd_config
echo "PubkeyAuthentication yes" >> %{buildroot}%{_sysconfdir}/ssh/sshd_config

# Configure to use strong MACs
echo "MACs hmac-sha2-256-etm@openssh.com,umac-128-etm@openssh.com,hmac-sha2-512-etm@openssh.com" >> %{buildroot}%{_sysconfdir}/ssh/sshd_config
echo "MACs hmac-sha2-256-etm@openssh.com,umac-128-etm@openssh.com,hmac-sha2-512-etm@openssh.com" >> %{buildroot}%{_sysconfdir}/ssh/ssh_config
# Configure to use strong encryption ciphers
echo "Ciphers aes256-ctr,aes192-ctr,aes128-ctr,aes128-gcm@openssh.com,aes256-gcm@openssh.com" >> %{buildroot}%{_sysconfdir}/ssh/sshd_config
echo "Ciphers aes256-ctr,aes192-ctr,aes128-ctr,aes128-gcm@openssh.com,aes256-gcm@openssh.com" >> %{buildroot}%{_sysconfdir}/ssh/ssh_config
# Configure to drop weak KexAlgorithms
echo "KexAlgorithms -ecdh-sha2-nistp256,ecdh-sha2-nistp384,ecdh-sha2-nistp521,diffie-hellman-group14-sha256" >> %{buildroot}%{_sysconfdir}/ssh/sshd_config
echo "KexAlgorithms -ecdh-sha2-nistp256,ecdh-sha2-nistp384,ecdh-sha2-nistp521,diffie-hellman-group14-sha256" >> %{buildroot}%{_sysconfdir}/ssh/ssh_config
# Configure to drop weak HostKeyAlgorithms
echo "HostKeyAlgorithms -ecdsa-sha2-nistp256" >> %{buildroot}%{_sysconfdir}/ssh/sshd_config
echo "HostKeyAlgorithms -ecdsa-sha2-nistp256" >> %{buildroot}%{_sysconfdir}/ssh/ssh_config
...
* Thu Dec 17 2024 Naveen Saini <naveen.kumar.saini@intel.com> - 9.8p1-5
- Enable key-based authentication & password authentication
- Disable banner

* Thu Dec 12 2024 Naveen Saini <naveen.kumar.saini@intel.com> - 9.8p1-4
- Drop weak macs algorithms as per ssh-audit recommendation
- Listed new macs algorithms.

* Fri Nov 29 2024 Naveen Saini <naveen.kumar.saini@intel.com> - 9.8p1-3
- Drop key-exchange algorithms ecdh-sha2-nistp*, diffie-hellman-group14-sha256 as per ssh-audit recommendation
- Drop host-key algorithm ecdsa-sha2-nistp256 as per ssh-audit recommendation
- Append recommended aes256-gcm@openssh.com, aes128-gcm@openssh.com cipher algorithms
