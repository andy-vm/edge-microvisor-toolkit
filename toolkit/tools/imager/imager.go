...
import (
	...
	ukiUtils "github.com/microsoft/azurelinux/toolkit/tools/imagegen/ukiutils"
...
func buildSystemConfig(systemConfig configuration.SystemConfig, disks []configuration.Disk, outputDir, buildDir string, imgContentFile string) (err error) {
	...
	const (
		...
		grub2Package         = "grub2"
		distroReleasePackage = "edge-release"
	)
	...
	if systemConfig.IsRootFS() {
		...
		diskDevPath, partIDToDevPathMap, partIDToFsTypeMap, isLoopDevice, encryptedRoot, err = setupDisk(buildDir, defaultTempDiskName, *liveInstallFlag, diskConfig, systemConfig.Encryption, systemConfig.EMTEncryptionEnabled)
		if err != nil {
			return
		}
	...
	if isOfflineInstall {
		...
		additionalExtraMountPoints := []*safechroot.MountPoint{
			...
			safechroot.NewMountPoint(*specDir, specMountPoint, "", safechroot.BindMountPointFlags, ""),
		}
		extraMountPoints = append(extraMountPoints, additionalExtraMountPoints...)
		...
		err = setupChroot.Run(func() error {
			return buildImage(mountPointMap, mountPointToFsTypeMap, mountPointToMountArgsMap, partIDToDevPathMap, partIDToFsTypeMap, mountPointToOverlayMap, packagesToInstall, systemConfig, diskDevPath, encryptedRoot, diffDiskBuild, imgContentFile, localRepoMountPoint, specMountPoint)
		})
		if err != nil {
		...
		err = buildImage(mountPointMap, mountPointToFsTypeMap, mountPointToMountArgsMap, partIDToDevPathMap, partIDToFsTypeMap, mountPointToOverlayMap, packagesToInstall, systemConfig, diskDevPath, encryptedRoot, diffDiskBuild, imgContentFile, localRepoMountPoint, specMountPoint)
		if err != nil {
			err = fmt.Errorf("failed to build image:\n%w", err)
			return
		}
...
func setupDisk(outputDir, diskName string, liveInstallFlag bool, diskConfig configuration.Disk, rootEncryption configuration.RootEncryption, EMTEncryptionEnabled bool) (diskDevPath string, partIDToDevPathMap, partIDToFsTypeMap map[string]string, isLoopDevice bool, encryptedRoot diskutils.EncryptedRootDevice, err error) {
	const (
		realDiskType = "path"
	)
	if diskConfig.TargetDisk.Type == realDiskType {
		if liveInstallFlag {
			...
			partIDToDevPathMap, partIDToFsTypeMap, encryptedRoot, err = setupRealDisk(diskDevPath, diskConfig,
				rootEncryption, false /*diskKnownToBeEmpty*/, EMTEncryptionEnabled)
		} else {
		...
		diskDevPath, partIDToDevPathMap, partIDToFsTypeMap, encryptedRoot, err = setupLoopDeviceDisk(outputDir, diskName, diskConfig, rootEncryption, EMTEncryptionEnabled)
		isLoopDevice = true
	}
	return
}

func setupLoopDeviceDisk(outputDir, diskName string, diskConfig configuration.Disk, rootEncryption configuration.RootEncryption, EMTEncryptionEnabled bool) (diskDevPath string, partIDToDevPathMap, partIDToFsTypeMap map[string]string, encryptedRoot diskutils.EncryptedRootDevice, err error) {
	defer func() {
	...
	partIDToDevPathMap, partIDToFsTypeMap, encryptedRoot, err = setupRealDisk(diskDevPath, diskConfig, rootEncryption,
		true /*diskKnownToBeEmpty*/, EMTEncryptionEnabled)
	if err != nil {
...
func setupRealDisk(diskDevPath string, diskConfig configuration.Disk, rootEncryption configuration.RootEncryption, diskKnownToBeEmpty bool, EMTEncryptionEnabled bool,
	...
	partIDToDevPathMap, partIDToFsTypeMap, encryptedRoot, err = diskutils.CreatePartitions(diskDevPath, diskConfig,
		rootEncryption, diskKnownToBeEmpty, EMTEncryptionEnabled)
	if err != nil {
...
func buildImage(mountPointMap, mountPointToFsTypeMap, mountPointToMountArgsMap, partIDToDevPathMap, partIDToFsTypeMap map[string]string,
	mountPointToOverlayMap map[string]*installutils.Overlay, packagesToInstall []string,
	systemConfig configuration.SystemConfig, diskDevPath string, encryptedRoot diskutils.EncryptedRootDevice,
	diffDiskBuild bool, imgContentFile string, localRepoMountPoint string, specMountPoint string,
) (err error) {
	...
	err = installutils.PopulateInstallRoot(installChroot, packagesToInstall, systemConfig, mountList, mountPointMap,
		...
		diffDiskBuild, *genEMTList, localRepoMountPoint, specMountPoint)
	if err != nil {
	...
	if !systemConfig.IsRootFS() {
		err = installutils.ConfigureDiskBootloader(systemConfig.BootType, systemConfig.Encryption.Enable,
			...
			encryptedRoot, systemConfig.EnableGrubMkconfig, false, systemConfig.EfiProvider, systemConfig.EMTEncryptionEnabled)
		if err != nil {
	...
	err = installutils.CreateResolvCfgSymlink(installChroot.RootDir())
	if err != nil {
		err = fmt.Errorf("failed to create /etc/resolv.conf symlink: %s", err)
		return
	}
	...
	// Build & install Unified Kernel Image
	if systemConfig.EfiProvider == systemdBoot {
		err = ukiUtils.InstallUnifiedKernelImage(installChroot)
		if err != nil {
			err = fmt.Errorf("failed to build unified kernel image: %s", err)
			return
		}
	}
