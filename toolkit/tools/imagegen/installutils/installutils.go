...
const (
	...
	// grubEfi represents GRUB bootloader, systemdBoot represents systemd-boot bootloader.
	grubEfi     = "grub-efi"
...
func PopulateInstallRoot(installChroot *safechroot.Chroot, packagesToInstall []string,
	...
	diffDiskBuild bool, genEMTList bool, localRepoMountPoint string, specDir string,
) (err error) {
	...
	installedPackages, err := calculateTotalPackages(packagesToInstall, installRoot, genEMTList, localRepoMountPoint, specDir)
	if err != nil {
		return
	}
...
func configureSystemFiles(installChroot *safechroot.Chroot, hostname string, config configuration.SystemConfig,
	...
	err = UpdateFstab(installChroot.RootDir(), config.PartitionSettings, mountList, mountPointMap,
		...
		config.ImmutabilityEnabled, config.EMTEncryptionEnabled)
	if err != nil {
		return
	}
...
func calculateTotalPackages(packages []string, installRoot string, genEMTList bool,
	localRepoMountPoint string, specDir string,
) (installedPackages *repocloner.RepoContents, err error) {
	var (
		releaseverCliArg string
	)
	...
	for _, pkg := range packages {
		...
		for _, line := range splitStdout {
			...
			substdout, substderr, suberr := shell.Execute("tdnf", "info", releaseverCliArg, "--nogpgcheck", matches[tdnf.InstallPackageName])
			if suberr != nil {
				err = fmt.Errorf("%v", substderr)
				return
			}

			// process raw data to extract URL and license info
			subsplitStdout := strings.Split(substdout, "\n")
			url := ""
			license := ""
			// go thru each line to get URL and License info
			for _, subline := range subsplitStdout {
				switch {
				case tdnf.ReURL.MatchString(subline):
					url = strings.TrimSpace(tdnf.ReURL.FindStringSubmatch(subline)[1])
				case tdnf.ReLicense.MatchString(subline):
					license = strings.TrimSpace(tdnf.ReLicense.FindStringSubmatch(subline)[1])
				}

				// dont need to go thru all the lines once info found
				if url != "" && license != "" {
					break
				}
			}
			...
			// get modified status
			if genEMTList {
				toFind := "Edge Microvisor Toolkit"
				notfound := false
				cleanVersion := matches[tdnf.InstallPackageVersion]
				// somehow some noarch has : in tdnf version but not in rpm file name
				if matches[tdnf.InstallPackageArch] == "noarch" {
					parts := strings.Split(cleanVersion, ":")
					if len(parts) > 1 {
						cleanVersion = parts[1]
					}
				}
				rpmName := localRepoMountPoint + "/" + matches[tdnf.InstallPackageName] + "-" + cleanVersion +
					"." + matches[tdnf.InstallPackageDist] + "." + matches[tdnf.InstallPackageArch]
				specName, rpmerror := rpm.QueryRPMSpecName(rpmName)
				if rpmerror != nil {
					logger.Log.Warnf("Package %v specName not found: %v", matches[tdnf.InstallPackageName], rpmerror)
					notfound = true
				}

				distrib, distriberr := rpm.FindDistributionInSpec(specDir, specName)
				if distriberr != nil {
					logger.Log.Warnf("Package %v distribution not found: %v", matches[tdnf.InstallPackageName], distriberr)
					notfound = true
				}

				// by default not modified, and not found if theres error in rpm
				if notfound {
					pkg.Modified = "Not Found"
				} else {
					pkg.Modified = "No"
				}

				if strings.Contains(distrib, toFind) {
					// set to modified if Edge Microvisor Toolkit is present in distribution
					pkg.Modified = "Yes"
				}
			}
...
func AddImageIDFile(installChrootRootDir string, buildNumber string, imageIDContents ...string) (err error) {
	// // If liveInstallFlag is provided and set to true, do not create the image-id file
	// if len(liveInstallFlag) > 0 && liveInstallFlag[0] {
	// 	return nil
	// }
	...
	imageIDContent := ""
	if len(imageIDContents) > 0 && imageIDContents[0] != "" {
		imageIDContent = imageIDContents[0]
	} else {
...
func UpdateFstab(installRoot string, partitionSettings []configuration.PartitionSetting, mountList []string,
	...
	hidepidEnabled bool, immutabilityEnabled bool, EMTEncryptionEnabled bool,
) (err error) {
	...
	return UpdateFstabFile(fullFstabPath, partitionSettings, mountList, mountPointMap,
		...
		hidepidEnabled, immutabilityEnabled, EMTEncryptionEnabled)
}
...
func UpdateFstabFile(fullFstabPath string, partitionSettings []configuration.PartitionSetting, mountList []string,
	...
	for _, mountPoint := range mountList {
		...
		if mountPoint != "" && devicePath != NullDevice {
			...
			err = addEntryToFstab(fullFstabPath, mountPoint, devicePath, mountPointToFsTypeMap[mountPoint],
				mountPointToMountArgsMap[mountPoint], partSetting.MountIdentifier, !doPseudoFsMount, immutabilityEnabled, false)
			if err != nil {
				return
			}
		}
	...
	if hidepidEnabled {
		err = addEntryToFstab(fullFstabPath, "/proc", "proc", "proc", "rw,nosuid,nodev,noexec,relatime,hidepid=2", configuration.MountIdentifierNone, doPseudoFsMount, immutabilityEnabled, false)
		if err != nil {
			return
		}
	}

	if EMTEncryptionEnabled {
		err = addEntryToFstab(fullFstabPath, "swap", "/dev/mapper/swap_crypt", "swap", "defaults,nofail,x-systemd.device-timeout=1s,ro", configuration.MountIdentifierNone, doPseudoFsMount, immutabilityEnabled, true)
		if err != nil {
			return
		}
	}
	...
	for partID, fstype := range partIDToFsTypeMap {
		if fstype == "linux-swap" {
			...
			if exists {
				err = addEntryToFstab(fullFstabPath, "none", swapPartitionPath, "swap", "", "", doPseudoFsMount, immutabilityEnabled, false)
				if err != nil {
					return
				}
			}
...
func addEntryToFstab(fullFstabPath, mountPoint, devicePath, fsType, mountArgs string, identifierType configuration.MountIdentifier, doPseudoFsMount bool, immutabilityEnabled bool, EMTEncryptionEnabled bool) (err error) {
	const (
		rootfsMountPoint = "/"
		...
		EMTEncryptPass   = "1"
		defaultPass      = "2"
		persistPartLabel = "edge_persistent"
	)

	// Skip fstab entry for persistent partition
	var partLabel string
	if strings.Contains(devicePath, "dev") && devicePath != "/dev/mapper/swap_crypt" {
		partLabel, err = GetPartLabel(devicePath)
		if err != nil {
			logger.Log.Warnf("Failed to get partition Label for block device %v", devicePath)
			return
		}
		if partLabel == persistPartLabel {
			return
		}
	}
	...
	// Set rootfs mount option to readonly
	if immutabilityEnabled && mountPoint == rootfsMountPoint {
...
func ConfigureDiskBootloader(bootType string, encryptionEnable bool, partitionSettings []configuration.PartitionSetting,
	...
	includeLegacyGrubCfg bool, efiProvider string, EMTEncryptionEnabled bool,
) (err error) {
	...
	return ConfigureDiskBootloaderWithRootMountIdType(bootType, encryptionEnable, rootMountIdentifier, kernelCommandLine,
		...
		efiProvider, EMTEncryptionEnabled)
}
...
func ConfigureDiskBootloaderWithRootMountIdType(bootType string, encryptionEnable bool,
	...
	if efiProvider == systemdBoot {
		// Prepare boot configuration for UKI
		err = installGrubTemplateFile(resources.AssetsBootParmsDefFile, tmpBootParmsCfgFile, installChroot.RootDir(), rootDevice, bootUUID,
			bootPrefix, encryptedRoot, kernelCommandLine, isBootPartitionSeparate, EMTEncryptionEnabled, mountPointMap)
		if err != nil {
			logger.Log.Warnf("Failed to form boot parameters for UKI (%s): %v", tmpBootParmsCfgFile, err)
			return
		}
	} else {
		...
		err = InstallGrubDefaults(installChroot.RootDir(), rootDevice, bootUUID, bootPrefix, encryptedRoot,
			kernelCommandLine, isBootPartitionSeparate, includeLegacyGrubCfg, mountPointMap)
		if err != nil {
...
func InstallGrubDefaults(installRoot, rootDevice, bootUUID, bootPrefix string,
	...
	isBootPartitionSeparate bool, includeLegacyCfg bool, mountPointMap map[string]string,
) (err error) {
	...
	err = installGrubTemplateFile(resources.AssetsGrubDefFile, GrubDefFile, installRoot, rootDevice, bootUUID,
		bootPrefix, encryptedRoot, kernelCommandLine, isBootPartitionSeparate, false, mountPointMap)
	if err != nil {
...
func installGrubTemplateFile(assetFile, targetFile, installRoot, rootDevice, bootUUID, bootPrefix string,
	encryptedRoot diskutils.EncryptedRootDevice, kernelCommandLine configuration.KernelCommandLine,
	isBootPartitionSeparate bool, EMTEncryptionEnabled bool, mountPointMap map[string]string,
) (err error) {
	...
	// Add Encryption boot_uuid
	err = setEncryptionBootUUID(installGrubDefFile, EMTEncryptionEnabled, mountPointMap)
	if err != nil {
		logger.Log.Warnf("Failed to set encryption bootUUID in %s: %v", installGrubDefFile, err)
		return
	}

	// Add rd.auto
	err = setrdAuto(installGrubDefFile, EMTEncryptionEnabled)
	if err != nil {
		logger.Log.Warnf("Failed to set rd.auto in %s: %v", installGrubDefFile, err)
		return
	}
...
func setEncryptionBootUUID(bootCfgPath string, setEncryptionBootUUID bool, mountPointMap map[string]string) (err error) {
	const (
		bootUUIDPrefix  = "boot_uuid="
		bootUUIDPattern = "{{.EncryptionBootUUID}}"
	)
	var (
		cmdline  configuration.KernelCommandLine
		bootUUID string
	)
	if setEncryptionBootUUID {
		var id string
		id, err = GetUUID(mountPointMap[rootMountPoint])
		if err != nil {
			err = fmt.Errorf("failed to get UUID: %s", err)
			return
		}

		bootUUID = fmt.Sprintf("%v%v", bootUUIDPrefix, id)
	}
	logger.Log.Debugf("Adding EncryptionBootUUID('%s') to '%s'", bootUUID, bootCfgPath)
	err = sed(bootUUIDPattern, bootUUID, cmdline.GetSedDelimeter(), bootCfgPath)
	if err != nil {
		logger.Log.Warnf("Failed to set boot cfg's bootUUID: %v", err)
		return
	}
	return
}

func setrdAuto(bootCfgPath string, setEncryptionBootUUID bool) (err error) {
	const (
		rdAutoPrefix  = "rd.auto="
		rdAutoPattern = "{{.rdAuto}}"
	)
	var (
		cmdline configuration.KernelCommandLine
		rdAuto  string
	...
	if !setEncryptionBootUUID {
		rdAuto = fmt.Sprintf("%v%v", rdAutoPrefix, "1")
	}
	logger.Log.Debugf("Adding rd.auto('%s') to '%s'", rdAuto, bootCfgPath)
	err = sed(rdAutoPattern, rdAuto, cmdline.GetSedDelimeter(), bootCfgPath)
	if err != nil {
		logger.Log.Warnf("Failed to set boot cfg's rd.auto: %v", err)
		return
	}
	return
}
...
// Create /etc/resolv.conf symlink if does not exist
func CreateResolvCfgSymlink(installChrootRootDir string) (err error) {
	// Check if /etc directory exists and it does not, throw an error
	_, err = os.Stat(filepath.Join(installChrootRootDir, "/etc"))
	if err != nil {
		if os.IsNotExist(err) {
	...
	// Check if /etc/resolv.conf file exists and if it does, then nothing to do, return
	_, err = os.Stat(filepath.Join(installChrootRootDir, "/etc/resolv.conf"))
	if err != nil {
		if os.IsExist(err) {
			return
		}
	...
	ReportAction(fmt.Sprintf("Creating /etc/resolv.conf symlink"))

	resolvCfg := filepath.Join(installChrootRootDir, "/etc/resolv.conf")
	_, stderr, err := shell.Execute("ln", "-sf", "../run/systemd/resolve/stub-resolv.conf", resolvCfg)
	if err != nil {
		logger.Log.Warnf(stderr)
		return
	}
